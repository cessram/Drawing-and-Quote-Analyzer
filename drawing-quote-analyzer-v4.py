...
def parse_equipment_from_text(text):
    ...
    item_re = re.compile(r'^(\d{1,2}[a-z]?)\s+(.+)$', re.IGNORECASE)
    equip_num_re = re.compile(r'^(\d{4,}\.?\d*)\s+(.+)$')
    qty_cat_re = re.compile(r'\s+(\d+)\s+([1-8])\s*$')
    qty_only_re = re.compile(r'\s+(\d+)\s*$')
    ...
    for line in lines:
        ...
        letter_count = len(re.sub(r'[^a-zA-Z]', '', rest))
        if letter_count < 5:
            continue
        ...
        if qty_cat:
            ...
            qty_only = qty_only_re.search(before_elec)
            if qty_only:

...
def parse_crs_quote_from_text(text, filename):
    """Parse CRS-style quote from PDF text - handles Canadian Restaurant Supply format
    Format: Item | Qty | Description | Sell | Sell Total
    Example: '4    1 ea  WALK IN     $X,XXX.XX  $X,XXX.XX'
    """
    quotes = []
    lines = text.split('\n')
    
    current_item = None  # This is the "Item" column that matches drawing "No."
    current_qty = 0
    current_desc = ""
    ...
    # Item column in quotes matches No. column in drawings
...
def extract_quotes_from_dataframe(df, filename):
    ...
    Quote 'Item' column matches Drawing 'No.' column
    """
    quotes = []
    
    df.columns = df.columns.astype(str).str.strip().str.lower()
    
    # Column mapping for quotes - "Item" in quotes matches "No." in drawings
    # Based on CRS format: Item | Qty | Description | Sell | Sell Total
    col_mapping = {
    ...
    for _, row in df.iterrows():
        try:
            ...
            # Get Item value (this matches Drawing's "No." column)
            ...
            try:
                unit_str = str(unit_price).replace('
...
def process_quote_file(parsed_data):
    ...
    # Remove duplicates based on Item (which matches Drawing "No.")
    seen_items = set()
    unique_quotes = []
    for q in quotes:
...
def match_quote_to_schedule(schedule_item, all_quotes):
    """Find matching quote for a schedule item
    PRIMARY match: Drawing "No." column ↔ Quote "Item" column
    """
...
def analyze_schedule_vs_quotes(equipment_schedule, all_quotes):
    ...
    Matches Drawing "No." to Quote "Item"
    """
    analysis = []
    ...
    for item in equipment_schedule:
        ...
        analysis.append({
            'No': item['No'],  # Drawing "No." column
            'Quote_Item': matched_quote['Item'] if matched_quote else '-',  # Quote "Item" column
...
with tabs[0]:
    ...
    with col2:
        ...
        st.info("Upload one or more quotation files (CRS format supported). 'Item' column matches Drawing 'No.' column. Supported formats: PDF, CSV, XLSX")
...
with tabs[2]:
    if st.session_state.equipment_schedule and st.session_state.quotes_data:
        ...
        display_df = filtered_df[['No', 'Quote_Item', 'Description', 'Schedule_Qty', 'Quote_Qty', 
    ...
    for i, line in enumerate(lines):
        ...
        if match:
            ...
            if current_item and current_desc:
                quotes.append({
                    'Item': current_item,  # Quote "Item" matches Drawing "No."
...
, '').replace(',', '').strip()
            ...
            total_price = row.get(found_cols.get('total_price', ''), 0)
            try:
                total_str = str(total_price).replace('
...
, '').replace(',', '').strip()
            ...
            quotes.append({
                'Item': item_val,  # Quote "Item" matches Drawing "No."
                'Description': desc_val,
                'Qty': qty_val,
                'Unit_Price': unit_price,
                'Total_Price': total_price,

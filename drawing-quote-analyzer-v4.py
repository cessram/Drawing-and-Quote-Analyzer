...
def parse_crs_quote_from_text(text, filename):
    ...
    # Pattern 1: Full line with prices - "2    1 ea  WALK IN    $97,980.27  $97,980.27"
    # Item# + Qty + ea + Description + $Price + $Total
    full_pattern = re.compile(
        r'^(\d+[a-z]?)\s+(\d+)\s*ea\s+(.+?)\s+\$?([\d,]+\.\d{2})\s+\$?([\d,]+\.\d{2})\s*
...
<div style='text-align: center; color: #666; padding: 20px;'>
    <p>Equipment Quote Analyzer v5.4 | Built for Bird Construction</p>
...
,
    ...
    # Pattern 2: Line with single price - "2    1 ea  WALK IN    $97,980.27"
    single_price_pattern = re.compile(
        r'^(\d+[a-z]?)\s+(\d+)\s*ea\s+(.+?)\s+\$?([\d,]+\.\d{2})\s*
...
,
    ...
    # Pattern 3: Line without prices - "2    1 ea  WALK IN"
    no_price_pattern = re.compile(
        r'^(\d+[a-z]?)\s+(\d+)\s*ea\s+([A-Z][A-Z\s,\./&\-\(\)\']+)',
        re.IGNORECASE
    )
    
    # Pattern for NIC items (Not In Contract) - skip these
    nic_pattern = re.compile(r'^(\d+[a-z]?)\s+NIC\b', re.IGNORECASE)
    
    # Pattern for ITEM TOTAL line
    total_pattern = re.compile(r'ITEM\s*TOTAL[:\s]*\$?([\d,]+\.\d{2})', re.IGNORECASE)
    ...
    for line in lines:
        ...
        # Try full pattern first (with both prices)
